...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $nim = $_POST['nim'];
    $nama = $_POST['nama'];
    $presensi = $_POST['status_presensi'];
    $count = count($nim);
    $sql = "INSERT INTO presensi (tgl_presensi,makul,kelas,nim,nama,status_presensi) VALUES ";
    for ($i = 0; $i < $count; $i++) {
        $sql .= "(sysdate(),'$makul','$kelas','$nim[$i]}','{$nama[$i]}','$presensi')";
        $sql .= ",";
    }
    $sql = rtrim($sql, ",");
...
<head>
    ...
    <title>Dosen | Presensi Mahasiswa</title>
...
<body class="bg-dark">
    ...
    <div class="card card-register mx-auto mt-5">
      <div class="card-header text-center"><h4>Pengisian Presensi Mahasiswa</h4></div>
      <div class="card-body">
...
<p class="text-center">
        ...
        <div class="text-center">
          <br>Copyright © DinaBerliana  <?=date('Y');?><br>
        </div>
      </div>
    </div>
</body>
